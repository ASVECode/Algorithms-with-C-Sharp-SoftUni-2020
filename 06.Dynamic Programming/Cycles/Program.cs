...
namespace Cycles
{
    class Program
    {
        ...
        static OrderedBag<Magnum.Collections.Tuple<string, string>> edges = new OrderedBag<Magnum.Collections.Tuple<string, string>>();
        ...
        public static void Main()
        {
            ...
            while (!string.IsNullOrEmpty(line))
            {
                ...
                {
                    edges.Add(new Magnum.Collections.Tuple<string, string>(vertex[0], vertex[i]));
                }

                line = Console.ReadLine();
            }

            var result = new List<Magnum.Collections.Tuple<string, string>>();
            foreach (var edge in edges)
            {
                ...
                if (needToRemove)
                {
                    if (!result.Contains(new Magnum.Collections.Tuple<string, string>(child, parent)))
                    {
                        result.Add(edge);
                    }
                }
